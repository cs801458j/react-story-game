import stage_0_0_1 from '../images/game/stage_0_0_1.jpeg';
import stage_0_1_0 from '../images/game/stage_0_1_0.jpeg';
import stage_0_1_1 from '../images/game/stage_0_1.JPG';
import stage_0_1_2 from '../images/game/stage_0_1_2.jpeg';
import stage_0_1_3 from '../images/game/stage_0_1_3.jpeg';
import stage_0_1_4 from '../images/game/stage_0_1_4.jpeg';
import stage_0_1_5 from '../images/game/stage_0_1_5.jpeg';
import stage_0_2 from '../images/game/stage_0_2.JPG';
import stage_0_3 from '../images/game/stage_0_3.JPG';
import stage_0_4 from '../images/game/stage_0_4.jpeg';
import stage_0_5 from '../images/game/stage_0_5.jpeg';
import stage_0_6 from '../images/game/stage_0_6.jpeg';
import stage_0_7 from '../images/game/stage_0_7.jpeg';
import stage_0_8 from '../images/game/stage_0_8.jpeg';
import stage_0_9 from '../images/game/stage_0_9.jpeg';
import stage_0_10 from '../images/game/stage_0_10.jpeg';
import stage_0_11 from '../images/game/stage_0_11.jpeg';
import stage_0_12 from '../images/game/stage_0_12.jpeg';
import stage_0_13 from '../images/game/stage_0_13.jpeg';
import stage_0_14 from '../images/game/stage_0_14.jpeg';
import stage_0_15 from '../images/game/stage_0_15.jpeg';
import stage_0_16 from '../images/game/stage_0_16.jpeg';
import stage_0_17 from '../images/game/stage_0_17.jpeg'; //  악마버섯
import stage_0_18 from '../images/game/stage_0_18.jpeg';
import stage_0_19 from '../images/game/stage_0_19.jpeg';
import stage_0_20 from '../images/game/stage_0_20.jpeg';
import stage_0_bed from '../images/game/stage_0_bed.jpeg';

import black from '../images/game/black.jpeg';
// import selection from '../images/game/selection.jpeg';
import date_1 from '../images/game/date_1.JPG';
import date_2 from '../images/game/date_2.JPG';
import selection from '../images/game/selection.jpeg';
//  stage 1
import stage_1_1 from '../images/game/stage_1_1.jpeg';
import stage_1_rampung_0 from '../images/game/stage_1_rampung_0.jpeg';
import stage_1_rampung_1 from '../images/game/stage_1_rampung_1.jpeg';
import stage_1_reporter_1 from '../images/game/stage_1_reporter_1.jpeg';
import stage_1_reporter_2 from '../images/game/stage_1_reporter_2.jpeg';
import stage_1_lovegame_1 from '../images/game/stage_1_lovegame_1.jpeg';
import stage_1_lovegame_2 from '../images/game/stage_1_lovegame_2.jpeg';
import stage_1_lovegame_3 from '../images/game/stage_1_lovegame_3.jpeg';
import stage_1_lovegame_4 from '../images/game/stage_1_lovegame_4.jpeg';
import stage_1_lovegame_5 from '../images/game/stage_1_lovegame_1.JPG';

import stage_1_lovegame_6 from '../images/game/stage_1_lovegame_7.jpeg';
import stage_1_lovegame_7 from '../images/game/stage_1_lovegame_2.JPG';
import stage_1_lovegame_sbs from '../images/game/stage_1_lovegame_sbs.jpeg';
import stage_1_staff_1 from '../images/game/stage_1_staff_1.jpeg';
import stage_1_staff_2 from '../images/game/stage_1_staff_2.jpeg';
import stage_1_kbs from '../images/game/stage_1_kbs.jpeg';
//  stage 2

import stage_2_talk_0 from '../images/game/stage_2_talk_0.jpeg';
import stage_2_talk_0_0 from '../images/game/stage_2_talk_0.jpeg';
import stage_2_talk_1 from '../images/game/stage_2_talk_1.jpeg';
import stage_2_talk_2 from '../images/game/stage_2_talk_2.jpeg';
import stage_2_talk_3 from '../images/game/stage_2_talk_3.jpeg';
import stage_2_talk_4 from '../images/game/stage_2_talk_4.jpeg';
import stage_2_talk_5 from '../images/game/stage_2_talk_5.jpeg';
import stage_2_talk_6 from '../images/game/stage_2_talk_6.jpeg';
import stage_2_talk_7 from '../images/game/stage_2_talk_7.jpeg';
import stage_2_talk_8 from '../images/game/stage_2_talk_8.jpeg';
// 본 스토리
import stage_2_health_0_0 from '../images/game/stage_2_health_0_0.jpeg';
import stage_2_health_0_1 from '../images/game/stage_2_health_0_1.jpeg';
import stage_2_health_0_3 from '../images/game/stage_2_health_0_3.jpeg';
import stage_2_health_0_4 from '../images/game/stage_2_health_0_4.jpeg';
import stage_2_health_0_5 from '../images/game/stage_2_health_0_5.jpeg';
import stage_2_health_0_6 from '../images/game/stage_2_health_0_6.jpeg';
import stage_2_health_0_7 from '../images/game/stage_2_health_0_7.jpeg';
// import stage_2_health_1 from '../images/game/stage_2_health_1.JPG';
import stage_2_health_2 from '../images/game/stage_2_health_2.JPG';
import stage_2_chkmate_1 from '../images/game/stage_2_chkmate_1.JPG';
import stage_2_chkmate_2 from '../images/game/stage_2_chkmate_2.jpeg';
import stage_2_chkmate_3 from '../images/game/stage_2_chkmate_3.JPG';
import stage_2_chkmate_4 from '../images/game/stage_2_chkmate_4.JPG';
import stage_2_topmedia_1 from '../images/game/stage_2_topmedia_1.JPG';
import stage_2_topmedia_2 from '../images/game/stage_2_topmedia_2.JPG';
import stage_2_topmedia_3 from '../images/game/stage_2_topmedia_3.JPG';
import stage_2_topmedia_4 from '../images/game/stage_2_topmedia_4.JPG';
import stage_2_eat_mushroom from '../images/game/stage_2_eat_mushroom.jpeg';

// 3 talk
import stage_3_talk_0 from '../images/game/stage_3_talk_0.jpeg';
import stage_3_talk_1 from '../images/game/stage_3_talk_1.jpeg';
import stage_3_talk_2 from '../images/game/stage_3_talk_2.jpeg';
import stage_3_talk_3 from '../images/game/stage_3_talk_3.jpeg';
import stage_3_talk_4 from '../images/game/stage_3_talk_4.jpeg';

//  stage 3
import stage_3_vlive_1 from '../images/game/stage_3_vlive_1.JPG';
import stage_3_vlive_2 from '../images/game/stage_3_vlive_2.JPG';
import stage_3_vlive_3 from '../images/game/stage_3_vlive_3.JPG';
import stage_3_concert_1 from '../images/game/stage_3_concert_1.JPG';
import stage_3_concert_2 from '../images/game/stage_3_concert_2.JPG';
import stage_3_concert_3 from '../images/game/stage_3_concert_3.JPG';
import stage_3_concert_4 from '../images/game/stage_3_concert_4.JPG';
import stage_3_bubble_1 from '../images/game/stage_3_bubble_1.JPG';
import stage_3_bubble_2 from '../images/game/stage_3_bubble_2.JPG';
import stage_3_bubble_3 from '../images/game/stage_3_bubble_3.JPG';
import stage_3_bubble_4 from '../images/game/stage_3_bubble_4.JPG';
import stage_3_bubble_5 from '../images/game/stage_3_bubble_5.JPG';

import stage_3_end_0 from '../images/game/stage_3_end_0.jpeg';
import stage_3_end_1 from '../images/game/stage_3_end_1.jpeg';
import stage_3_end_2 from '../images/game/stage_3_end_2.jpeg';
import stage_3_end_3 from '../images/game/stage_3_end_3.jpeg';
import stage_3_end_4 from '../images/game/stage_3_end_4.jpeg';
import stage_3_end_5 from '../images/game/stage_3_end_5.jpeg';
import stage_3_end_6 from '../images/game/stage_3_end_6.jpeg';
import stage_3_end_7 from '../images/game/stage_3_end_7.jpeg';
import stage_3_end_8 from '../images/game/stage_3_end_8.jpeg';

// stage title
import stage_1 from '../images/game/stage_1.jpeg';
import stage_2 from '../images/game/stage_2.jpeg';
import stage_3 from '../images/game/stage_3.jpeg';

// 메인 게임 스토리 라인
export const gameStory = [
  {
    stageId: 0,
    stageTitle: '어느 날 눈 떠보니 내가 다람쥐?',
    contents: [
      {
        character: '',
        sentence: '2022.06.06 오후 11시 30분.',
        image: date_1,
      },
      {
        character: '',
        sentence: '누워있던 진혁은 부엌에서 풍기는 맛있는 냄새에 눈이 번쩍 뜨였다.',
        image: stage_0_0_1,
      },
      {
        character: '',
        sentence: '냄새를 따라가 보니 그곳엔... 어머니께서 라면을 드시고 계시는 것이었다.',
        image: stage_0_1_0,
      },
      {
        character: '진혁',
        sentence: '엄마 지금 라면드셔요?',
        image: stage_0_1_1,
      },
      {
        character: '엄마',
        sentence: 'ㅇ',
        image: stage_0_1_1,
      },
      {
        character: '진혁',
        sentence: '냄새 엄청나서 잠 깼어. 배고파서.',
        image: stage_0_1_2,
      },
      {
        character: '엄마',
        sentence: '넌 왜그렇게 코가발달됬냐',
        image: stage_0_1_3,
      },
      {
        character: '진혁',
        sentence: '어쩌지... 나 지금 밀가루 안 먹는데...',
        image: stage_0_1_3,
      },
      {
        character: '엄마',
        sentence: '괜찮아. 버섯 라면은 밀가루 아니고 버섯 99.9% 야.',
        image: stage_0_1_4,
      },
      {
        character: '진혁',
        sentence: '진짜요? 그럼 그냥 먹고 잘까.',
        image: stage_0_1_4,
      },
      {
        character: '엄마',
        sentence: '그래. 이건 어차피 살도 안쪄.',
        image: stage_0_1_5,
      },
      {
        character: '',
        sentence: '결국 진혁은 버섯 라면을 두 봉지 끓여서 맛있게 먹었다.',
        image: stage_0_2,
      },
      {
        character: '',
        sentence: '그리고 행복한 마음으로 방에 돌아가 바로 잠들었다.',
        image: stage_0_3,
      },
      {
        character: '',
        sentence: '2022.06.07 8:00 am',
        image: date_2,
      },
      {
        character: '진혁',
        sentence: '으음.. 일어나야 하는데',
        image: black,
      },
      {
        character: '진혁',
        sentence: '근데.. 내 침대가 이렇게 컸나?',
        image: black,
      },
      {
        character: '',
        sentence:
          '침대 옆 책상머리맡에 둔 안경이 이렇게 멀었나 생각하며 안경을 쓴 진혁. 손을 보고 화들짝 놀라며 거울을 들었다.',
        image: stage_0_bed,
      },
      {
        character: '',
        sentence: '거울에 비친 모습은 이진혁... 아니 다람쥐 한 마리였다.',
        image: stage_0_4,
      },
      {
        character: '진람쥐',
        sentence: '큰일이다… 내일은 내 생일이라 팬들도 만나야 하구..ㅜ 어쩌면 좋지',
        image: stage_0_5,
      },
      {
        character: '',
        sentence: '그 순간, 방 한구석에서 펑 하는 소리가 발생했다.',
        image: stage_0_6,
      },
      {
        character: '진람쥐',
        sentence: '아니 이게 무슨 소리지?',
        image: stage_0_6,
      },
      {
        character: '',
        sentence: '고개를 돌린 그곳엔 빛이 나는 무언가가 있었다.',
        image: stage_0_7,
      },
      {
        character: '',
        sentence: '점점 다가오는 어떤 생명체.',
        image: stage_0_8,
      },
      {
        character: '',
        sentence: '그곳엔 거대 버섯요정이 있었다.',
        image: stage_0_9,
      },
      {
        character: '????',
        sentence: '왔구나.',
        image: stage_0_10,
      },
      {
        character: '진혁',
        sentence: '넌 누구야?',
        image: stage_0_11,
      },
      {
        character: '버섯요정',
        sentence: '난 버섯라면의 저주에 걸린 사람들만 만날 수 있다는 버섯요정이야.',
        image: stage_0_13,
      },
      {
        character: '진혁',
        sentence: '? 지금 니 말은 내가 저주에 걸렸다고?',
        image: stage_0_14,
      },
      {
        character: '버섯요정',
        sentence:
          'ㅇㅇ 넌 어젯밤에 버섯라면을 두 봉지나 끓여 먹었기 때문에 악마버섯의 저주에 걸렸어.',
        image: stage_0_15,
      },
      {
        character: '진혁',
        sentence: '아니 왜?? 내가 배고파서 먹을 수도 있지 ㅡㅡ 거 참 쪼잔하넹',
        image: stage_0_15,
      },
      {
        character: '버섯요정',
        sentence: '맞아 그럴 수도 있지... 하지만 넌 엄마의 탈을 쓴 악마버섯 시험에 넘어간 거야!',
        image: stage_0_12,
      },
      {
        character: '진혁',
        sentence: '어젯밤... 나랑 얘기 한 사람이 엄마가 아니고 악마버섯이었다구?!',
        image: stage_0_17,
      },
      {
        character: '버섯요정',
        sentence: '그래. 드디어 상황 파악이 됐구나?',
        image: stage_0_16,
      },
      {
        character: '진혁',
        sentence: '그러면 내가... 사람으로 돌아가려면 어떻게 해야 하니 버섯요정아ㅠㅠ',
        image: stage_0_18,
      },
      {
        character: '버섯요정',
        sentence: '지금부터 세 가지 미션을 통과하면 널 다시 사람으로 돌려줄게.',
        image: stage_0_19,
      },
      {
        character: '진혁',
        sentence: '좋아. 그 미션 완벽하게 클리어 하겠어!',
        image: stage_0_20,
      },
    ],
  },
  {
    stageId: 1,
    stageTitle: '느나 만나기',
    contents: [
      {
        character: 'STAGE 1',
        sentence: '그래! 일단 친한 누나를 만나서 상황을 어떻게 해결해야 할지 물어보자.',
        image: stage_1,
      },
      {
        character: '',
        sentence: '근데 어떤 누나한테 가지?',
        image: stage_1,
      },
      {
        character: '선택',
        sentence: '골라보자.',
        image: selection,
      },
    ],
  },
  {
    stageId: 2,
    stageTitle: '어디로든 버섯(문)',
    contents: [
      {
        character: 'STAGE 2',
        sentence: '좋아. 그러면 버섯을 먹고 어디로 갈지 생각하면 되는 거지?',
        image: stage_2,
      },
      {
        character: '',
        sentence: '근데 어디로 가지?',
        image: stage_2,
      },
      {
        character: '선택',
        sentence: '골라보자.',
        image: selection,
      },
    ],
  },
  {
    stageId: 3,
    stageTitle: '브이단에게 말하자!',
    contents: [
      {
        character: 'STAGE 3',
        sentence: '그래 ... 숨기고만 있을 순 없으니까 빨리 말하자.',
        image: stage_3,
      },
      {
        character: '',
        sentence: '근데 어디에서 말해주지? ㅠㅠ',
        image: stage_3,
      },
      {
        character: '선택',
        sentence: '골라보자',
        image: selection,
      },
    ],
  },
];

//  게임 선택지
export const gameSelection = [
  {
    stageId: 1,
    selections: [
      '주접이풍년 미선누나 만나기',
      'sbs 라디오 소현누나 만나기',
      '왜 오수재인가 서현진 누나 만나기',
    ],
    answer: 1,
  },
  {
    stageId: 2,
    selections: ['헬스장에 간다', '작업실에 간다', '회사로 간다'],
    answer: 1,
  },
  {
    stageId: 3,
    selections: ['브이앱', '콘서트장', '버블'],
    answer: 1,
  },
];

// 1단계 스토리
export const stageStory = [
  [
    {
      stageId: 1,
      subStageId: 0,
      index: 0,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 미선느나를 만나러 주접이풍년으로 가자!',
          image: stage_1_kbs,
        },
        {
          character: '',
          sentence: '그렇게 주접이풍년 촬영장으로 찾아간 진람쥐.',
          image: stage_1_kbs,
        },
        {
          character: '진람쥐',
          sentence: '안녕하세요~ 혹시 미선느나 지금 어디에 계세요?',
          image: stage_1_staff_1,
        },
        {
          character: '스탭',
          sentence: '여 마침 잘 왔다.',
          image: stage_1_staff_1,
        },
        {
          character: '스탭',
          sentence:
            '안 그래도 오늘 미선누님이 몸이 안 좋아서 MC 대타가 급하게 필요했는데 네가 하면 되겠다.',
          image: stage_1_1,
        },
        {
          character: '진람쥐',
          sentence: '??? 저 지금 다람쥔데요???',
          image: stage_1_staff_2,
        },
        {
          character: '스탭',
          sentence: '그게 중요한 게 아니야 지금 ... 빨리 스탠바이 하자.',
          image: stage_1_staff_2,
        },
        {
          character: '',
          sentence: '그렇게 ... 진람쥐는 다람쥐인 채로 주접리포터로 주접이 풍년에 나가게 되었다.',
          image: stage_1_reporter_1,
        },
        {
          character: '',
          sentence: '다람쥐 리포터는 특유의 귀여움으로 소소하게 화제가 되었다.',
          image: stage_1_reporter_2,
        },
        {
          character: '',
          sentence: '하지만 미선느나를 만나지 못해 결국 조언을 얻지 못했다.',
          image: stage_1_reporter_2,
        },
      ],
    },
    {
      stageId: 1,
      subStageId: 1,
      index: 1,
      result: 'success',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 소현느나를 만나러 SBS 라디오로 가자!',
          image: stage_1_lovegame_sbs,
        },
        {
          character: '',
          sentence: '그렇게 목동 SBS로 찾아간 진람쥐.',
          image: stage_1_lovegame_sbs,
        },
        {
          character: '진람쥐',
          sentence: '안녕하세요~ 혹시 소현느나 지금 있나요?',
          image: stage_1_lovegame_1,
        },
        {
          character: '소현누나',
          sentence: '어머! 혹시 너 진혁이니?',
          image: stage_1_lovegame_2,
        },
        {
          character: '진람쥐',
          sentence:
            '녜 누나 ... 저 어떡하죠. 자고 일어나보니 다람쥐가 됐어요ㅠㅠ 이런 몸으로 방송을 할 수 있을까요? ㅠㅠ',
          image: stage_1_lovegame_5,
        },
        {
          character: '소현누나',
          sentence: '진람.. 아니 진혁아 괜찮아 다람쥐면 어때? 너라는 사실이 중요한거지',
          image: stage_1_lovegame_5,
        },
        {
          character: '진람쥐',
          sentence: '정말인가요ㅠㅠ 저 다람쥐여도 괜찮을까요? 팬들이 싫어하면 어쩌죠',
          image: stage_1_lovegame_5,
        },
        {
          character: '소현누나',
          sentence:
            '다람쥐인 지금도 충분히 귀엽고 사랑스러운걸! 오늘 찾아온 김에 같이 게스트로 이야기하다 갈래?',
          image: stage_1_lovegame_4,
        },
        {
          character: '진람쥐',
          sentence: '조금 부끄럽지만 ... 좋아요!',
          image: stage_1_lovegame_4,
        },
        {
          character: '',
          sentence: '그렇게 진람쥐는 러브게임 최초의 다람쥐 게스트가 되었다.',
          image: stage_1_lovegame_3,
        },
        {
          character: '',
          sentence:
            '다람쥐로 변해도 즐겁고 유쾌한 진람쥐의 토크는 107.7 MHz과 고릴라를 통해 전국으로 퍼져갔다.',
          image: stage_1_lovegame_3,
        },
        {
          character: '',
          sentence: 'MISSION SUCCESS',
          image: stage_1_lovegame_6,
        },
        {
          character: '소현누나',
          sentence: '축하해. 미션을 통과했구나.',
          image: stage_1_lovegame_7,
        },
        {
          character: '소현누나',
          sentence:
            '너는 다람쥐인 모습으로도 용기 있게 라디오에 출연했기 때문에 이 버섯을 받을 자격이 있어.',
          image: stage_1_lovegame_7,
        },
        {
          character: '획득',
          sentence: "진람쥐는 '용기의 버섯'을(를) 획득했다 !",
          image: stage_1_lovegame_7,
        },
        {
          character: '버섯요정',
          sentence: '축하해. 용기의 버섯을 획득했구나.',
          image: stage_2_talk_0,
        },
        {
          character: '진람쥐',
          sentence: '용기의 버섯? 뭐야 이거 뭐 일곱 개 모아야 사람 되고 ... 그런거야?',
          image: stage_2_talk_1,
        },
        {
          character: '버섯요정',
          sentence: '그건 드래곤볼이고... 앞으로 두 가지 버섯만 모으면 넌 사람이 될 수 있어.',
          image: stage_2_talk_2,
        },
        {
          character: '진람쥐',
          sentence: '그러면 다음 버섯을 모으려면 어떻게 해야 하니 버섯요정아?',
          image: stage_2_talk_3,
        },
        {
          character: '버섯요정',
          sentence: '후훗. 그건 바로바로 ~ 쨘!',
          image: stage_2_talk_4,
        },
        {
          character: '',
          sentence: '펑 하는 소리와 함께 나타난 그것은 ... 바로 작고 소중한 버섯이었다.',
          image: stage_2_talk_5,
        },
        {
          character: '진람쥐',
          sentence: '?? 이게 뭐야.',
          image: stage_2_talk_6,
        },
        {
          character: '버섯요정',
          sentence: '이건 바로 먹으면 어디로든 갈 수 있는 어디로든 버섯이야.',
          image: stage_2_talk_7,
        },
        {
          character: '진람쥐',
          sentence: '이거 그냥 어디로든 문 짭 아니야?',
          image: stage_2_talk_8,
        },
        {
          character: '버섯요정',
          sentence: '큼큼.. 무슨 그런 섭섭한 버섯... 아니 섭섭한 소리를',
          image: stage_2_talk_8,
        },
        {
          character: '버섯요정',
          sentence:
            '아무튼 이 버섯을 먹으면서 네가 가고 싶은 곳을 마음 속 깊이 생각하면 그곳으로 이동할 수 있어!',
          image: stage_2_talk_8,
        },
      ],
    },
    {
      stageId: 1,
      subStageId: 2,
      index: 2,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 현진느나를 만나러 왜 오수재인가 촬영장으로 가자!',
          image: stage_1_rampung_0,
        },
        {
          character: '',
          sentence: '그렇게 왜 오수재인가 촬영장으로 찾아간 진람쥐.',
          image: stage_1_rampung_0,
        },
        {
          character: '진람쥐',
          sentence: '안녕하세요~ 혹시 현진느나 지금 현장에 계신가요?',
          image: stage_1_staff_1,
        },
        {
          character: '스탭',
          sentence: '오늘은 학교씬이 아니고 법정씬이여서 여기는 없을텐데... ',
          image: stage_1_1,
        },
        {
          character: '스탭',
          sentence: '마침 잘됐다. 추가 컷으로 찍어야 할 부분 온 김에 찍고 가자.',
          image: stage_1_1,
        },
        {
          character: '진람쥐',
          sentence: '??? 저 지금 다람쥔데요???',
          image: stage_1_staff_2,
        },
        {
          character: '스탭',
          sentence: '됐고 일단 빨리 대기하자.',
          image: stage_1_staff_2,
        },
        {
          character: '',
          sentence: '그렇게 .. 진람쥐는 다람쥐인 채로 왜 오수재인가를 촬영했다.',
          image: stage_1_rampung_1,
        },
        {
          character: '',
          sentence: '진람쥐는 람춘풍으로 로스쿨 최초 다람쥐 학생으로 출연하게 됐다.',
          image: stage_1_rampung_1,
        },
        {
          character: '',
          sentence: '하지만 다람쥐 대학생은 한국 사회의 정서상 시청자들이 받아들이기 어려웠다... ',
          image: stage_1_rampung_1,
        },
      ],
    },
  ],
  [
    {
      stageId: 2,
      subStageId: 0,
      index: 0,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 사람으로 돌아가려면 체력이 먼저지 ... 헬스장으로 가자.',
          image: black,
        },
        {
          character: '',
          sentence: '진람쥐는 버섯을 꾹 삼켰다.',
          image: stage_2_eat_mushroom,
        },
        {
          character: '',
          sentence: '눈을 떠보니.. 빠른 비트의 노래소리가 들려왔다.',
          image: stage_2_health_0_0,
        },
        {
          character: '???',
          sentence: '회원님. 오셨군요.',
          image: stage_2_health_0_1,
        },
        {
          character: '진람쥐',
          sentence: '형, 저 왔어요. 근데 저 다람쥐가 됐어요 ㅠㅠ',
          image: stage_2_health_0_3,
        },
        {
          character: '짐종국',
          sentence: '그건 됐고 일단 운동을 시작해보자.',
          image: stage_2_health_0_3,
        },
        {
          character: '진람쥐',
          sentence: '앗... 녭',
          image: stage_2_health_0_4,
        },
        {
          character: '',
          sentence: '그렇게 .. 진람쥐는 열심히 PT를 받았다.',
          image: stage_2_health_0_5,
        },
        {
          character: '',
          sentence:
            '종국이  형은 다람쥐가 된 걸 신경도 안 쓰는 것처럼 보였다. 그저 헬스만 계속 시켰다...',
          image: stage_2_health_0_6,
        },
        {
          character: '',
          sentence: '~ 3개월 후 ~',
          image: stage_2_health_0_7,
        },
        {
          character: '',
          sentence: '혹독한 PT로 근육빵빵 다람쥐가 된 진람쥐.',
          image: stage_2_health_2,
        },
        {
          character: '',
          sentence: '그렇게 몸짱 다람쥐가 되어 맨즈헬스 최초 다람쥐 모델이 되었다.',
          image: stage_2_health_2,
        },
      ],
    },
    {
      stageId: 2,
      subStageId: 1,
      index: 1,
      result: 'success',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 일단 곡 작업도 해야 하니까 체크메이트 형한테 가보자.',
          image: black,
        },
        {
          character: '',
          sentence: '진람쥐는 버섯을 꾹 삼켰다.',
          image: stage_2_eat_mushroom,
        },
        {
          character: '진람쥐',
          sentence: '형... 큰일났어... 나 다람쥐가 됐어.',
          image: stage_2_chkmate_1,
        },
        {
          character: '체크메이트',
          sentence: '다람쥐가 됐다고?',
          image: stage_2_chkmate_1,
        },
        {
          character: '체크메이트',
          sentence: '음 ... 근데 다람쥐면 어때? 네 목소리만 그대로면 된 거 아니야? ',
          image: stage_2_chkmate_1,
        },
        {
          character: '진람쥐',
          sentence: '그런가? 정말 내 목소리만 그대로면 될까?',
          image: stage_2_chkmate_2,
        },
        {
          character: '체크메이트',
          sentence: '당연하지. 아니 오히려 귀여운 거 같기도? 그러니까 일단 녹음하자.',
          image: stage_2_chkmate_2,
        },
        {
          character: '',
          sentence:
            '그렇게 5집 수록곡 녹음을 하게 된 진람쥐. 마이크 높이는 낮아졌지만 실력은 여전했다.',
          image: stage_2_chkmate_3,
        },
        {
          character: '',
          sentence: '진람쥐는 최상의 컨디션으로 수록곡 녹음을 마쳤다.',
          image: stage_2_chkmate_3,
        },
        {
          character: '',
          sentence: 'MISSION SUCCESS',
          image: stage_2_chkmate_3,
        },
        {
          character: '체크메이트',
          sentence: '축하해. 미션을 통과했구나.',
          image: stage_2_chkmate_4,
        },
        {
          character: '체크메이트',
          sentence:
            '너는 다람쥐로 변해도 본업에 충실한 모습 덕분에 능력의 버섯을 받을 자격이 있어.',
          image: stage_2_chkmate_4,
        },
        {
          character: '획득',
          sentence: "진람쥐는 '능력의 버섯'을(를) 획득했다 !",
          image: stage_2_chkmate_4,
        },
        {
          character: '버섯요정',
          sentence: '축하해. 능력의 버섯을 획득했구나. ',
          image: stage_2_talk_0_0,
        },
        {
          character: '버섯요정',
          sentence: '이제 벌써 마지막 단계야. 마지막으로 네가 해야 할 일을 알려줄게. ',
          image: stage_3_talk_0,
        },
        {
          character: '진람쥐',
          sentence: '그래. 내가 마지막으로 뭘 하면 될까?',
          image: stage_3_talk_0,
        },
        {
          character: '버섯요정',
          sentence:
            '그건 바로 팬들에게 네가 다람쥐가 된 걸 알리는 거야. 생각해봐. 팬들은 아직 모르고 있잖아?',
          image: stage_3_talk_1,
        },
        {
          character: '진람쥐',
          sentence: '맞아... 이젠 브이단도 내가 다람쥐가 된 걸 알아야겠지?',
          image: stage_3_talk_2,
        },
        {
          character: '진람쥐',
          sentence: '하지만 두려워 ... 내가 다람쥐로 변했어도 우리 브이단이 나를 사랑해줄까? ',
          image: stage_3_talk_3,
        },
        {
          character: '버섯요정',
          sentence: '영원한 비밀은 없으니까 차라리 빨리 말하는 게 나을 거 같은데?',
          image: stage_3_talk_4,
        },
      ],
    },
    {
      stageId: 2,
      subStageId: 2,
      index: 2,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 일단 회사에 알려야겠다.',
          image: black,
        },
        {
          character: '',
          sentence: '진람쥐는 버섯을 꾹 삼켰다.',
          image: stage_2_eat_mushroom,
        },
        {
          character: '',
          sentence: '회사에 도착한 진람쥐는 두리번거리며 주변을 확인했다.',
          image: stage_2_topmedia_1,
        },
        {
          character: '진람쥐',
          sentence: '휴 ... 다행히 아무도 없군.',
          image: stage_2_topmedia_1,
        },
        {
          character: '진람쥐',
          sentence: '여러분 저 큰일났어요... 저 컴백을 못하게 생겼어요.',
          image: stage_2_topmedia_2,
        },
        {
          character: '티오피미디어 직원1',
          sentence: '근데 ... 누구세요? ',
          image: stage_2_topmedia_3,
        },
        {
          character: '진람쥐',
          sentence: '저를 못 알아보면 어떡해요ㅜㅜ 저 진혁이에요.',
          image: stage_2_topmedia_3,
        },
        {
          character: '티오피미디어 직원2',
          sentence: '아! 목소리 들으니 알 것 같다.',
          image: stage_2_topmedia_4,
        },
        {
          character: '티오피미디어 직원1',
          sentence: '근데 진혁이? 뭐야 왜 귀여워짐?',
          image: stage_2_topmedia_4,
        },
        {
          character: '진람쥐',
          sentence: '아니 그게 아니고 저 다람쥐라서 컴백을 못 한다니까요?',
          image: stage_2_topmedia_4,
        },
        {
          character: '티오피미디어 직원2',
          sentence: '헐 근데 너무 작고 귀여워ㅠㅠ 람쥐야 같이 셀카찍자.',
          image: stage_2_topmedia_4,
        },
        {
          character: '',
          sentence: '그렇게 직원들에게 귀여움만 받은 채 소속 다람쥐 아이돌이 된 진람쥐.',
          image: stage_2_topmedia_4,
        },
        {
          character: '',
          sentence: '종종 동물농장 패널로 진람쥐를 볼 수 있었다는 소문이 전설처럼 전해졌다고 한다.',
          image: stage_2_topmedia_4,
        },
      ],
    },
  ],
  [
    {
      stageId: 3,
      subStageId: 0,
      index: 0,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 많은 브이단이 보려면 역시 브이앱이지!',
          image: black,
        },
        {
          character: '',
          sentence: '그렇게 브이앱을 켠 진람쥐.',
          image: stage_3_vlive_1,
        },
        {
          character: '진람쥐',
          sentence: '여러분 ... 많이 놀라셨죠. 저 진혁이에요',
          image: stage_3_vlive_1,
        },
        {
          character: '진람쥐',
          sentence: '아니 제가 어제 버섯라면을 먹고 잤는데ㅜ 자고 일어나보니 이렇게 변했더라구요',
          image: stage_3_vlive_1,
        },
        {
          character: '',
          sentence: '브이앱 댓글창은 폭발했다. 그런데 ... 반응이',
          image: stage_3_vlive_1,
        },
        {
          character: '',
          sentence:
            "'뭐야 티오피 브이앱으로 CG도 가능했어???' / '대박 직원 새로 뽑음?' 처럼 다른 의미의 폭발적인 반응이었다.",
          image: stage_3_vlive_2,
        },
        {
          character: '진람쥐',
          sentence: '아니 여러분. 이거 CG 아니고 진짜 저예요. 저 다람쥐가 됐다구요!',
          image: stage_3_vlive_2,
        },
        {
          character: '',
          sentence:
            "브이앱은 순식간에 화제가 됐다. 다양한 커뮤니티에 '아이돌 브이앱에 CG 적용하는 회사.jpg'로 퍼져나간 진람쥐 브이앱",
          image: stage_3_vlive_3,
        },
        {
          character: '',
          sentence: '결국 아무도 진람쥐가 다람쥐로 변했다는 사실을 믿어주지 않았다.',
          image: stage_3_vlive_3,
        },
      ],
    },
    {
      stageId: 3,
      subStageId: 1,
      index: 1,
      result: 'success',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 콘서트장에서 말하기로 결심했어!',
          image: black,
        },
        {
          character: '진람쥐',
          sentence: '내가 다람쥐로 변한 모습도 좋... 좋아해줄까',
          image: black,
        },
        {
          character: '',
          sentence: '- 콘서트장 -',
          image: stage_3_concert_2,
        },
        {
          character: '진람쥐',
          sentence: '휴 ... 무대는 언제해도 긴장된당. 근데 다람쥐로 올라가려니 더 떨리네.',
          image: stage_3_concert_2,
        },
        {
          character: '',
          sentence: '팬들의 함성소리를 들으며 백스테이지에서 말하기 시작하는 진람쥐',
          image: stage_3_concert_1,
        },
        {
          character: '진람쥐',
          sentence: '브이단!!! 너무너무 오랜만이죠?',
          image: stage_3_concert_1,
        },
        {
          character: '진람쥐',
          sentence:
            '저도 무대에서 여러분을 볼 수 있어서 반가워요 ㅠㅠ 그런데 여러분에게 꼭 해야 할 말이 있어요.',
          image: stage_3_concert_1,
        },
        {
          character: '진람쥐',
          sentence:
            '사실 ... 제가 지난 밤 버섯라면을 먹고 잤는데 글쎄 자고 일어나보니 다람쥐로 변해버렸어요.',
          image: stage_3_concert_1,
        },
        {
          character: '진람쥐',
          sentence: '그래서 변해버린 제 모습에 여러분이 당황할까 봐 무대 뒤에서 말을 하고 있어요.',
          image: stage_3_concert_1,
        },
        {
          character: '진람쥐',
          sentence:
            '제가 다람쥐로 변해버려도... 브이단은 저를 아직 사랑하시나요? 다람쥐인 채로 무대를 해도 괜찮을까요.',
          image: stage_3_concert_1,
        },
        {
          character: '',
          sentence:
            '콘서트장은 잠시 정적이 흘렀다. 그러나 이윽고 괜찮다는 말과 어떤 모습이더라도 사랑한다는 함성으로 가득 채워졌다.',
          image: stage_3_concert_3,
        },
        {
          character: '',
          sentence:
            '진람쥐는 멋진 다람쥐의 모습으로 팬들 앞으로 나와 콘서트를 성공적으로 마무리했다.',
          image: stage_3_concert_3,
        },
        {
          character: '',
          sentence: '브이단의 응원으로 자신감을 얻게 된 진람쥐',
          image: stage_3_concert_3,
        },
        {
          character: '획득',
          sentence: "진람쥐는 '응원의 버섯'을(를) 획득했다 !",
          image: stage_3_concert_4,
        },
        {
          character: '',
          sentence: 'MISSION SUCCESS',
          image: stage_3_concert_4,
        },
        {
          character: '버섯요정',
          sentence: '축하해. 세 가지 버섯을 모두 모았구나.',
          image: stage_3_end_0,
        },
        {
          character: '진람쥐',
          sentence: '맞아. 나를 응원해주는 사람들 덕분에 이 버섯들을 다 모을 수 있었어.',
          image: stage_3_end_1,
        },
        {
          character: '버섯요정',
          sentence: '그럼. 항상 너를 응원하는 사람들이 많다는 걸 잊지 말라구! ',
          image: stage_3_end_2,
        },
        {
          character: '버섯요정',
          sentence: '자, 약속한 대로 널 사람으로 돌려줄게. ',
          image: stage_3_end_3,
        },
        {
          character: '진람쥐',
          sentence: '고마워, 버섯요정아! 그리고 너도 잊지 않을게.',
          image: stage_3_end_4,
        },
        {
          character: '',
          sentence: '펑! 하는 소리와 함께 진람쥐. 아니 진혁은 현실로 돌아왔다.',
          image: stage_3_end_5,
        },
        {
          character: '진혁',
          sentence: '휴~ 다행이얌. 이제 다시 나로 돌아왔구나!',
          image: stage_3_end_6,
        },
        {
          character: '',
          sentence:
            '그렇게 사람으로 돌아온 진혁이는 27살 생일을 브이단과 행복하게 보낼 수 있었다고 한다.',
          image: stage_3_end_7,
        },
        {
          character: '',
          sentence: '- THE END -',
          image: stage_3_end_8,
        },
      ],
    },
    {
      stageId: 3,
      subStageId: 2,
      index: 2,
      result: 'fail',
      contents: [
        {
          character: '진람쥐',
          sentence: '그래. 우선 버블로 브이단에게 말해보자',
          image: black,
        },
        {
          character: '',
          sentence: '그렇게 버블을 보내는 진람쥐.',
          image: stage_3_bubble_1,
        },
        {
          character: '진람쥐',
          sentence: "'모행~?'",
          image: stage_3_bubble_2,
        },
        {
          character: '진람쥐',
          sentence: "'다름이 아니라 ... 나 다람쥐가 되어썽 ㅜㅜ'",
          image: stage_3_bubble_2,
        },
        {
          character: '진람쥐',
          sentence: '휴우~ 떨린당 다들 모라고 답장했을까?',
          image: stage_3_bubble_2,
        },
        {
          character: '',
          sentence:
            "'?? 오늘 만우절 아닌데??' / '우리 지녀기 많이 심심하구나? ㅎㅎ'/ '점메추 ㅂㅌ'",
          image: stage_3_bubble_3,
        },
        {
          character: '',
          sentence: '진람쥐의 기대와는 사뭇 다른 버블들이 ... 도착했다. ',
          image: stage_3_bubble_3,
        },
        {
          character: '진람쥐',
          sentence: "'아니 그게 아니구 나 진짜로 다람쥐가 됐다니까?!'",
          image: stage_3_bubble_4,
        },
        {
          character: '',
          sentence: "'서럽다 ... 이제 강쥐단은 사람도 아니다? '/ '이제 람지단 공식됨 ㅇㅇ'",
          image: stage_3_bubble_5,
        },
        {
          character: '진람쥐',
          sentence: '하... 나 진짜 다람쥐 맞는데 🥺🥺🥺',
          image: stage_3_bubble_5,
        },
        {
          character: '',
          sentence: '브이단이 아무도 믿어주지 않아 ... 결국 혼자 만우절 장난친 다람쥐가 된 진람쥐.',
          image: stage_3_bubble_5,
        },
      ],
    },
  ],
];

// 첫 번째 선택지 스토리 라인
